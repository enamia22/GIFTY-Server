...
const addProduct = async (req, res,) => {
    ...
    try {
      ...
      if (       
         !productName || !shortDescription || !longDescription || !subcategoryId || !price || !discountPrice || !options ) {
...
const getAllProducts = async (req, res) => {
  try {
    ...
    const fieldsToRetrieve = 'subcategoryId sku productName productImage shortDescription';
  ...
  try {
    ...
    if (check) {
      ...
      if (product) {
        ...
        if (existingProduct)
        return res.status(400).json({ error: "product already exits" });
        const updated = await Product.findByIdAndUpdate(productId, productUpdated, {
          new: true,
        });
